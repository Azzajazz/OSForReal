...
$TOOLCHAIN_PREFIX/$TARGET-as -c $SRC_DIR/arch/x86/boot/boot.s -o $BUILD_DIR/boot.o
$TOOLCHAIN_PREFIX/$TARGET-as -c $SRC_DIR/arch/x86/asm_stubs.s -o $BUILD_DIR/stubs.o
$TOOLCHAIN_PREFIX/$TARGET-gcc -c $SRC_DIR/arch/x86/boot/bootstrap.c -o $BUILD_DIR/bootstrap.o $CFLAGS
$TOOLCHAIN_PREFIX/$TARGET-gcc -c $SRC_DIR/kernel/kernel.c -o $BUILD_DIR/kernel.o $CFLAGS
$TOOLCHAIN_PREFIX/$TARGET-gcc -T link.ld -Wl,-Map=$BUILD_DIR/output.map -Wl,--gc-sections -o $BUILD_DIR/os-for-real.elf $CFLAGS -nostdlib $BUILD_DIR/boot.o $BUILD_DIR/stubs.o $BUILD_DIR/bootstrap.o $BUILD_DIR/kernel.o -lgcc
...
$TOOLCHAIN_PREFIX/$TARGET-gcc -c $SRC_DIR/arch/x86/boot/bootstrap.c -o $BUILD_DIR/bootstrap_test.o -DKERNEL_TEST $CFLAGS
$TOOLCHAIN_PREFIX/$TARGET-gcc -c $TESTS_DIR/runner.c -o $BUILD_DIR/tests.o $CFLAGS
$TOOLCHAIN_PREFIX/$TARGET-gcc -T link.ld -Wl,-Map=$BUILD_DIR/output_tests.map -o $BUILD_DIR/os-for-real-test.elf $CFLAGS -nostdlib $BUILD_DIR/boot.o $BUILD_DIR/stubs.o $BUILD_DIR/bootstrap_test.o $BUILD_DIR/tests.o -lgcc
